...
# ==============================================================================
# 1. SELF-CONTAINED MODEL DEFINITION (WideResNet-94-16)
# ==============================================================================

class WideBasicBlock(nn.Module):
    """WideResNet basic block."""
    def __init__(self, in_planes, planes, stride, dropout_rate):
...
class DecomposedWideResNet(nn.Module):
    """A self-contained, decomposable WideResNet-94-16."""
    def __init__(self, depth=94, widen_factor=16, dropout_rate=0.0, num_classes=10, quantization_split='layer3'):
        super(DecomposedWideResNet, self).__init__()
        self.in_planes = 16
    ...
    def _build_decomposed_model(self):
        """Splits the WRN into a feature extractor and a classifier."""
        # A standard WRN structure is conv1 -> layer1 -> layer2 -> layer3 -> bn1 -> relu -> avg_pool -> linear
        feature_layers = [self.conv1]
        classifier_layers = [self.layer3, self.bn1, nn.ReLU(inplace=True), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten(), self.linear]

        if self.quantization_split == 'layer1':
            feature_layers.append(self.layer1)
            classifier_layers = [self.layer2] + classifier_layers
        elif self.quantization_split == 'layer2':
            feature_layers.extend([self.layer1, self.layer2])
        elif self.quantization_split == 'layer3':
            feature_layers.extend([self.layer1, self.layer2, self.layer3])
            classifier_layers = [self.bn1, nn.ReLU(inplace=True), nn.AdaptiveAvgPool2d((1, 1)), nn.Flatten(), self.linear]
        else:
            raise ValueError(f"Invalid split point for WRN: {self.quantization_split}. Use 'layer1', 'layer2', or 'layer3'.")
...
def train_robust_model(model, train_loader, test_loader, device, args):
    """Trains a model using PGD Adversarial Training."""
...
def plot_latent_space(features_by_class, filename="latent_space_tsne_wide.png"):
    """
    ...
    plt.title("2D t-SNE Visualization of the Latent Space (WideResNet-94-16)")
...
def main():
    parser = argparse.ArgumentParser(description="Self-contained script to train a WideResNet and test its geometric separability.")
    ...
    # Analysis arguments
    parser.add_argument('--quantization_split', type=str, default='layer2', choices=['layer1', 'layer2', 'layer3'], help="Layer for latent space analysis.")
    ...
    parser.add_argument('--batch_size', type=int, default=64, help='Batch size for training and analysis. NOTE: WRN is memory intensive.')
    ...
    model = DecomposedWideResNet(
        depth=94,
        widen_factor=16,
        quantization_split=args.quantization_split,
        num_classes=10
    ).to(device)
    print(f"Initialized WideResNet-94-16. Total parameters: {sum(p.numel() for p in model.parameters())/1e6:.2f}M")
    ...
    print("\n" + "="*50)
    print("      DIAGNOSTIC EXPERIMENT RESULTS (WideResNet-94-16)")
    print("="*50)
